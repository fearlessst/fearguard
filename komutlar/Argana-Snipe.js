...
const data = require('quick.db')
  ...
  .setDescription(`<a:Follia_zz:801974860091228211> Mesaj bulunamadı!`)
.setColor(`#f3c7e1`)
    message.channel.send(embeds);
          } else {
  let kullanıcı = client.users.cache.get(emirhan);
  const Argana = await data.fetch(`snipe.mesaj.${message.guild.id}`)
  const embed = new MessageEmbed()
  .setAuthor(kullanıcı.username, kullanıcı.avatarURL())
  .setDescription(`<a:Follia_zz:801974860091228211> **Silinen mesaj :** ` + Argana)
.setColor(`#f3c7e1`)
