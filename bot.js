...
client.on("message", message => {
    ...
    return message.channel.send(`${message.author}, Aleyküm Selam Hoş Gelidn.`)
});


client.on("message", message => {
    if(message.content.toLowerCase() == "selam") 
    return message.channel.send(`${message.author}, Selam Hoş Geldin.`)
});


client.on("message", message => {
    if(message.content.toLowerCase() == "merhaba") 
    return message.channel.send(`${message.author}, Merhaba Hoş Geldin.`)
});


client.on("message", message => {
    if(message.content.toLowerCase() == "s.a") 
    return message.channel.send(`${message.author}, Aleyküm Selam Hoş Geldin.`)
});
...
let rol = member.guild.roles.cache.find(r => r.name === "Jail Rol İsim");
...
client.on('guildMemberAdd', async(member) => {
let mute = member.guild.roles.cache.find(r => r.name === "Mute Rol İsim");
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('Mute Rol ID');
  }, ms(süre));
}
})
 ...
 if(!db.has(`reklam_${msg.guild.id}`)) return;
        ...
        if (reklam.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.hasPermission("BAN_MEMBERS")) {
                  msg.delete();
                    return msg.channel.send(new Discord.MessageEmbed().setDescription(`<a:Follia_PatiSelam:801974928253124639> ${msg.author} Bu Sunucuda Reklam Yapmak Yasak <a:Follia_PatiSelam:801974928253124639>`).setColor('0x800d0d').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
